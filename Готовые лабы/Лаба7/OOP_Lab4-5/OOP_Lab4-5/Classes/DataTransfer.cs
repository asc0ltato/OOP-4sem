...
namespace OOP_Lab4_5
{
    public static class DataTransfer
    {
        ...
        public static void SerializeOld(Library books)
        {
            var serializer = new XmlSerializer(typeof(Library));
            using (var fs = new FileStream(@"C:\Users\user\Desktop\ООП\OOP_Lab4-5\OOP_Lab4-5\Resources\booksOld.xml", FileMode.Create))
            {
                serializer.Serialize(fs, books);
            }
        }
        ...
        public static Library DeserializeOld()
        {
            var serializer = new XmlSerializer(typeof(Library));
            Library deserializedFilmsList = null;

            using (var fs = new FileStream(@"C:\Users\user\Desktop\ООП\OOP_Lab4-5\OOP_Lab4-5\Resources\booksOld.xml", FileMode.OpenOrCreate))
            {
                if (fs.Length > 0)
                    deserializedFilmsList = serializer.Deserialize(fs) as Library;
            }
